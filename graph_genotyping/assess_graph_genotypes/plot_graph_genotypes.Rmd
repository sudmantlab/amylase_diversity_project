...
#t_ancient = read.table("/Users/petersudmant/Documents/science/sudmantlab/projects/ancient_DNA/amylase/amylase_diversity_project/graph_genotyping/tables/aDNA_best_geno_totv2.tsv",sep="\t",comment.char = "",col.names = c("sample", "besthaplo1_besthaplo2",  "best_score", "cohort")) %>% separate(besthaplo1_besthaplo2,into=c("hap1","hap2"),sep="\\$",extra="merge")
...
#t_HC_ancient_modpipe = read.table("/Users/petersudmant/Documents/science/sudmantlab/projects/ancient_DNA/amylase/amylase_diversity_project/graph_genotyping/tables/highcov_nea_MODdnapipe_bestgeno.tsv",sep="\t",comment.char = "",col.names = c("sample", "besthaplo1_besthaplo2",  "best_score", "cohort")) %>% separate(besthaplo1_besthaplo2,into=c("hap1","hap2"),sep="\\$",extra="merge")

#t_HC_ancient_apipe = read.table("/Users/petersudmant/Documents/science/sudmantlab/projects/ancient_DNA/amylase/amylase_diversity_project/graph_genotyping/tables/highcov_nea_adnapipe_bestgeno.tsv",sep="\t",comment.char = "",col.names = c("sample", "besthaplo1_besthaplo2",  "best_score", "cohort")) %>% separate(besthaplo1_besthaplo2,into=c("hap1","hap2"),sep="\\$",extra="merge")
...
#unique(hap1_bad$hap1)
#unique(hap2_bad$hap2)
#SEND LIST OF OVERSIGHT INDIVIDUALS TO ALE

# t %>% 
#   inner_join(t_clusts %>% transmute(hap1=sample, hap1_clust = cluster),on="hap1") %>%
#   inner_join(t_clusts %>% transmute(hap2=sample, hap2_clust = cluster),on="hap2") %>%
#   left_join(t_samples,on="sample")
# 
# t_samples %>% filter(sample == "HG00096")
...
# g=ggplot(t_HC_ancient_modpipe)
# g+geom_histogram(aes(x=best_score))+theme_bw()
# 
# g=ggplot(t_HC_ancient_apipe)
# g+geom_histogram(aes(x=best_score))+theme_bw()


# pipeline_compare = t_HC_ancient_apipe %>% transmute(sample=sample,best_score_apipe = best_score) %>%
#   inner_join(t_HC_ancient_modpipe %>% transmute(sample=sample,best_score_modpipe = best_score),by="sample")
# 
# g=ggplot(pipeline_compare)
# g+geom_point(aes(x=best_score_apipe,y=best_score_modpipe))+
#   geom_abline(slope=1)+
#   theme_bw()

t_plot = rbind(t_ancient %>% mutate(type="ancient"),t %>% mutate(type="modern")) %>%
          mutate(is_neo = grepl("NEO",sample)) %>%
          mutate(is_err = grepl("ERR",sample)) %>%
          mutate(is_prs = grepl("prs",sample))

g=ggplot(t_plot)
g+geom_histogram(aes(x=best_score,fill=is_prs))+
  #geom_histogram(aes(x=best_score,fill=is_err))+
  theme_bw()+
...
pipeline_compare %>% arrange(-best_score_apipe)
```
```{r}
