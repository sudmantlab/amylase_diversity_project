...
#########################
###SCORE CUTOFF
BEST_SCORE_CUTOFF = 0.75




g=ggplot(t %>% filter(sample %in% unique(t_cp$sample)))
g=g+geom_histogram(aes(x=best_score))+
  geom_vline(aes(xintercept=BEST_SCORE_CUTOFF),linetype="dotted",color="red")+
  theme_bw(base_size=6)+
  facet_wrap(~cohort,scales="free_y")+
  scale_x_continuous("best match score")

print(g)

fout = paste(outdir,"/deconvolution_scores.png",sep="")
png(fout, width=4 ,height=1.55,res=600,units="in")
print(g)
dev.off()


t %>% 
  filter(best_score>0.75) %>%
  group_by(cohort) %>%
  summarize(n=n())
...
fout = paste(outdir,"/copy_number_comparison.png",sep="")
png(fout, width=4 ,height=1.55,res = 400,units = 'in')
print(g)
dev.off()
...
colors = c("#EDB829", "#DD4124", "#00496F", "#94B669", "#E97C07" ,"purple","#0C7996")
#            AFR,        AMR,       CAS,      WEA,      SA,       OCN,       EA
pop_order = c("AFR", "AMR", "CAS", "WEA", "SA", "OCN", "EA")
...
t_plot$p2 = factor(t_plot$p2, levels = c(pop_order))
...
freq_sum_table = t_sum %>% 
  dplyr::select(p2,cluster,proportion) %>%
  mutate(proportion = signif(proportion,3)) %>%
  pivot_wider(names_from = "p2",values_from="proportion") %>%
  arrange(cluster)


write.table(freq_sum_table,
            file=paste(outdir,"/cluster_freqs_by_pop.tsv",sep=""),
            sep="\t",
            quote=FALSE,
            row.names = FALSE)
...
filtered_out = t_data %>% 
  filter(best_score>BEST_SCORE_CUTOFF) %>%
  mutate(haplotype=cluster,best_matching_score=best_score) %>%
  dplyr::select(sample, cohort, p1, p2, haplotype, hap, best_matching_score) %>%
  mutate(hap = ifelse(hap=="hap1_clust","H1","H2")) %>%
  pivot_wider(names_from=hap,values_from=haplotype)

write.table(filtered_out, file="/Users/petersudmant/Documents/science/sudmantlab/projects/ancient_DNA/amylase/amylase_diversity_project/graph_genotyping/assess_graph_genotypes/output/haplotype_deconvolution_filtered.tsv",sep="\t",row.names=FALSE,quote=FALSE)
...
t_subsistence = rbind(t_subsistence,additional_info_df) %>% unique()


t_data_by_sus = t_grouped %>% dplyr::select(sample,p1,hap,cluster) %>% 
              mutate(p1 = case_when(p1=="BEB" ~ "Bengali",
                                p1=="CLM" ~ "Colombian",
                                p1=="JPT" ~ "Japanese",
                                p1=="CDX" ~ "Dai",
                                p1=="CHB" ~ "Han",
...
#Cap the maximum number of individuals per population to 50
filtered_samples = t_data_by_sus %>% 
  dplyr::select(p1,sample) %>% 
  unique() %>%
  group_by(p1) %>%
  mutate(k = row_number()) %>%
  filter(k<=50) %>%
...
t_data_by_sus = inner_join(t_data_by_sus, filtered_samples, by="sample") %>% unique()
...
g=g+#geom_bar(aes(x=order,fill=type,y=proportion),stat='identity',position='dodge',alpha=0.8)+
  geom_bar(aes(x=type,fill=type,y=proportion),stat='identity',position='dodge',alpha=0.8)+
  theme_bw(base_size=5)+
  scale_fill_manual("",values=rev(brewer.pal(7,"Spectral")[c(1,2)]))+
...
contingency_table = t_data_by_sus %>% 
                    mutate(dup = ifelse(hap_names %in% c("H1^a","H2A0"), "not_dup", "dup")) %>%
                    group_by(type,dup) %>%
                    summarize(n = n()) %>%
                    pivot_wider(names_from=type,values_from=n) %>% 
                    column_to_rownames("dup")
...
t_hap_by_age = t_grouped %>%
  mutate(hap_names=cluster) %>%
   ...
   filter(p2 %in% c("WEA","EarlyFarmer","Hunter-GathererW","Hunter-GathererE","HunterGathererE","Yamnaya","Europe")) %>%
        left_join(ancient_info,by="sample") %>%
        mutate(age=ifelse(!cohort %in% c("StoneAgeAncient","ExcoffierAncient"),0,as.numeric(age))) %>% 
        filter(age<20000) %>%
        filter((cohort %in% c("1KG","SGDP","HGDP")) | (sample %in% haplotype_pred_copy_ancient$sample)) %>%
        mutate(age=-1*age) %>%
        mutate(time_bin = cut(age,breaks=c(-12000,-8000,-4000,-500,1),ordered_result = TRUE))
...
order = (t_sum_hap_by_age %>% dplyr::select(mu_freq,hap_names) %>% unique() %>% arrange(-mu_freq))$hap_names
t_sum_hap_by_age$hap_names = factor(t_sum_hap_by_age$hap_names, levels=order)

#g=ggplot(t_sum_hap_by_age %>% filter(mu_freq >0.01))
g=ggplot(t_sum_hap_by_age)
g=g+geom_point(aes(x=time_bin,y=proportion,size=count))+
  ...
  scale_size_continuous(range=c(0.1,1),breaks = c(1,seq(10,50,10)))+
  theme(legend.key.size=unit(0.1,"cm"))

fout = paste(outdir,"/ancient_hap_freqs_binned.png",sep="")
png(fout, width=3 ,height=1,units='in',res=400)
print(g)
dev.off()
...
```{r}
###SIMPLE BINS equally spaced to estimate trajectories with approxWF
...
t_hap_by_age = t_grouped %>%
   ...
   filter(p2 %in% c("WEA","EarlyFarmer","Hunter-GathererW","Hunter-GathererE","HunterGathererE","Yamnaya","Europe")) %>%
        ...
        mutate(time_bin = cut(age,breaks=c(-12000,  -8500,  -6000,  -3500,  -1000, 0),ordered_result = TRUE)) 
...
t_sum_hap_by_age_simple = t_hap_by_age %>%
          #filter(hap_names%in%c("H1^a", "H3^r", "H5","H4A2B2","H7","H2A0")) %>%
          group_by(time_bin) %>%
          mutate(gen_age = age/generation_time) %>%
          mutate(n_total = n(),
                 mu_gen = mean(gen_age)) %>%
...
t_hap_by_age %>% 
  filter(best_score>BEST_SCORE_CUTOFF) %>%
  mutate(group=ifelse(age==0,"modern","ancient")) %>%
  group_by(group) %>% 
  mutate(total=n()) %>%
  ungroup() %>%
  group_by(group,hap_names,total) %>% 
  summarize(n=n()) %>%
  mutate(prop = n/total) %>%
  arrange(prop) %>%
  filter(prop>0.05) %>%
  ungroup() %>%
  dplyr::select(hap_names) %>%
  unique()
...
#Note that the FIRST hap is what all others are compared against
...
t_s= read.table("/Users/petersudmant/Documents/science/sudmantlab/projects/ancient_DNA/amylase/amylase_diversity_project/graph_genotyping/assess_graph_genotypes/approxWF/input/haps_t_SIMPLE_10K.txt", header=T) %>% 
  ...
  filter(hap_names %in% c("s_HDUP"))  %>% mutate(hap_names = "H[dup]")
...
g=g+geom_density(aes(x=s,color=hap_names,fill=hap_names),alpha=0.25)+
  ...
  ggpp::geom_plot(data=data.tb, aes(x,y,label=plot),vp.width=(3/5)*1.1,vp.height=(1/2)*1.1)+
...
t_sum_hap_by_age_simple %>% 
  group_by(hap_names) %>%
  #filter(hap_names=="HDUP") %>% 
  mutate(mx=max(f),
         mn=min(f)) %>%
  mutate(fold_change = mx/mn) %>%
  arrange(hap_names)
