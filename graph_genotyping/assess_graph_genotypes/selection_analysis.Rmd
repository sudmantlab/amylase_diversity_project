...
#ggsave("figures/sample_age_distribution_plot.pdf", sample_age_distribution_plot, width=6, height=3)
```
...
sample_distribution_map <- ancient_map +
  geom_jitter(aes(fill=p2_for_plot, shape=p2_for_plot), size=2, color="black", alpha=0.8, height = 0.3, width = 0.3)
sample_distribution_map
#ggsave("figures/sample_distribution_map.pdf", sample_distribution_map, width=15, height = 7)
```
...
# ggsave("figures/copy_number_distribution.pdf", copy_number_distribution_plot, width=12, height = 4)
# ggsave("figures/copy_number_trend.pdf", copy_number_trend_plot, width=12, height = 4)
```
...
p_plot <- paths %>%
  ...
  scale_x_continuous(breaks = -4:0*3, labels = 4:0*3) +
  xlab("kyr BP") +
  ylab("frequency ofduplication-\ncontaining haplotypes") +
  theme_cowplot() +
  theme(axis.line = element_blank(),
        panel.border = element_rect(color="black", size=1))
combined_plot <- plot_grid(p_plot, s_plot , nrow = 2, align = "hv")
combined_plot
```

```{r eval=FALSE}
ggsave("figures/s_p_plot.pdf", combined_plot, width = 6, height = 6)
```
...
combined_heatmap <- plot_grid(NULL, top_histogram, NULL, NULL, 
          ...
          nrow = 3, rel_widths = c(0.5, 4, -0.3, 1.6), rel_heights = c(1.5, -0.2, 4), align ="hv", axis="tblr")
combined_heatmap
```
...
best_trajectories_plot <- bind_rows(best_trajectories, best_neutral_trajectories) %>% 
  ...
  annotate(geom = "label", x = c(-8.5, -8.9, -9.2), y=c(0.8, 0.3, 0.2), label=c("best neutral simulations", "best simulations", "real data"), color=c(MetBrewer::met.brewer(name = 'Archambault', type = "d", n = 2), "black")) +
  ...
  labs(x="kyr BP", y="frequency of duplication-containing haplotypes") +
  theme_bw() +
...
ggsave("figures/combined_heatmap.pdf", combined_heatmap, height=7, width = 8)
ggsave("figures/best_trajectories.pdf", height = 5, width=8)
```
...
```{r}
starting_frequency <- seq(from=0.05, to=0.55, length.out=21)
selection_coeff <- seq(from=-0.01, to=0.04, length.out=21)
selection_onset <- seq(from=1000, to=1400, length.out=21)
