...
t_wnd_raw = data.table::fread("/Users/petersudmant/Documents/science/sudmantlab/projects/ancient_DNA/amylase/input/amylase_1000_200.1000.200.tsv.gz",header=F,sep="\t",col.names = c("contig","start","end","cvg","sample")) %>%
  inner_join(stats,on="sample") %>%
  mutate(cp = 2*cvg/cvg_mu) %>%
  separate(sample,c("source","sample_id",'p1','p2'),remove=FALSE) %>%
  filter(start>0)
...
```{r}
modern_colors = pnw_palette("Bay",7)[1:7]
...
genotype_blocks = data.frame(name=c("AMY2B", "AMY2A","AMY1A","AMY1B","AMY1C"),
                             start=c(103564100,103612500,103638544,103684142,103732686),
                             end=  c(103588530,103617000,103667876,103712474,103762027))
...
#g=ggplot(t_wnd_raw %>% filter(sample %in% stats$sample[1:100]))
g=ggplot(t_wnd_raw)
g+geom_line(aes(x=start,y=cp,group=sample,color=p2),size=.1,alpha=.5)+
  #geom_segment(aes(x=end_1,xend=end_2,y=-1-ix*.5,yend=-1-ix*.5),size=.1,data=sds)+
  #geom_segment(aes(x=start_1,xend=end_1,y=-1-ix*.5,yend=-1-ix*.5,color=fracMatch),size=1,data=sds)+
  #geom_segment(aes(x=start_2,xend=end_2,y=-1-ix*.5,yend=-1-ix*.5,color=fracMatch),size=1,data=sds)+
  #geom_segment(aes(x=Start,xend=End,y=-1-ix*.5,yend=-1-ix*.5),color="green",size=.5,data=usher_blocks)+
  #geom_text(aes(x=Start,y=-1-ix*.5,label=Name),color="darkgreen",size=1.5,data=usher_blocks)+
  theme_bw()+
  facet_wrap(~p2,ncol=1,strip.position = "right")+
  scale_y_continuous(lim=c(0,7),breaks=seq(0,10,1))+
  theme(legend.position="None")+
  theme(strip.background = element_blank())+
  #scale_color_gradient2(low="grey",mid="yellow",high="orange")+
  #scale_color_gradient(low="grey",high="orange")+
  scale_color_manual(values=c(modern_colors))+
...
write.table(t_gt,fn_out,sep="\t",row.names = FALSE,quote=FALSE)
...
g+geom_histogram(aes(x=gt,fill=sample%in%off_samples),binwidth=0.05)+
  theme_bw(base_size=8)+
  #facet_wrap(~p2,scales="free_y",ncol=1)+
