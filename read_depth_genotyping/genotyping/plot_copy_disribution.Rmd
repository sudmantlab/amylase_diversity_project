...
n_modern = sample_counts %>% filter(!dataset %in% c("ExcoffierAncient", "StoneAgeAncient","Archaic")) %>% ungroup() %>% summarize(n=sum(n))
sample_counts
n_modern
...
g=g+geom_point(aes(x=type_label,y=cp,size=prop,color=type),alpha=0.8)+
  ...
  #geom_text(aes(x=0.25,y=xs,label=plabel),data=df_labs,parse=TRUE,size=2)+
  geom_text(aes(x=1.5,y=xs,label=plabel),data=df_labs,parse=TRUE,size=2,hjust=0)+
  ...
  coord_flip()+
  scale_x_discrete("")+
  scale_y_continuous("",breaks=c(c(0,1,2,3,4,5),seq(6,18,2)))+
...
# g=g+geom_point(aes(x=AMY1,y=AMY2B,size=n))+
#   theme_bw(base_size=8)+
#   scale_color_manual(values=colors)+
#   theme(legend.position="None")+
#   scale_x_continuous("AMY1",breaks=seq(0,20,2))
# 
# fn_out = "./output_figures/main_figs/copy_compare_AMY1_vs_AMY2B_sum.pdf"
# pdf(fn_out,width=2,height=1)
# print(g)
# dev.off()
...
dummy = data.frame(gene=c("AMY1","AMY2A","AMY2B"),y=c(2,2,-3))
