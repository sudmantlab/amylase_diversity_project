...
(88*87)/2
  ...
  t_gt_count %>% filter(dataset %in% c("SGDP"))
...
s##NOTE THERE ARE MORE! I just filtered out a bunch!
...
g=g+geom_point(aes(x=type_label,y=cp,size=prop,color=type),alpha=0.5)+
  ...
  geom_text(aes(x=1,y=xs,label=plabel),data=df_labs,parse=TRUE,size=3)+
...
t_plot_subset = t_plot %>% 
                  filter((n_total>=8)|(super_pop=="archaic"))


order = (t_plot_subset %>% ungroup() %>% filter(label=="AMY1") %>% dplyr::select(p2,p2_mu) %>% arrange(-p2_mu) %>% unique())$p2
...
df_vline_p2mu = t_plot_subset %>% filter(label=="AMY1") %>% dplyr::select(p2,p2_mu,label) %>% unique()


max(t_sum$mu)
g=ggplot(t_plot_subset %>% filter(label == "AMY1"))
g=g+geom_point(aes(x=cp,y=reorder(plabel,mu),size=prop,color=p2),alpha=.5)+
  ...
  geom_vline(aes(xintercept=p2_mu),data=df_vline_p2mu,linetype='85',size=.3,alpha=.75,color='red')+
...
pdf(fn_out,width=1.5,height=2.5)
print(g)
dev.off()
...
g=ggplot(t_plot_subset %>% filter(label == "AMY2A"))
g=g+geom_point(aes(x=cp,y=reorder(plabel,mu),size=prop,color=p2),alpha=.5)+
  geom_errorbarh(aes(xmin=mu-serr,xmax=mu+serr,y=reorder(plabel,mu)),size=.05,height=0,data=t_sum)+
  geom_point(aes(x=mu,y=reorder(plabel,mu)),shape=18,data=t_sum,size=.5)+
  geom_point(aes(x=mu,y=reorder(plabel,mu),color=p2),shape=18,data=t_sum,size=.05)+
...
g=ggplot(t_plot_subset %>% filter(label == "AMY2B") )
g=g+geom_point(aes(x=cp,y=reorder(plabel,mu),size=prop,color=p2),alpha=.5)+
  geom_errorbarh(aes(xmin=mu-serr,xmax=mu+serr,y=reorder(plabel,mu)),size=.05,height=0,data=t_sum)+
  geom_point(aes(x=mu,y=reorder(plabel,mu)),shape=18,data=t_sum,size=.5)+
  geom_point(aes(x=mu,y=reorder(plabel,mu),color=p2),shape=18,data=t_sum,size=.05)+
