...
t_cvg = rbind(t_wnd_cvg_dist,t_wnd_cvg_prox)
...
select_samples = summary_inf$sample[1:10]
...
g+#geom_histogram(aes(x=2*cvg/med),binwidth=.01)+
  geom_histogram(aes(x=cvg),binwidth=10)+
  #geom_vline(aes(xintercept=x),data=df_lines,color="orange",size=.1)+
  #geom_text(aes(x=5,y=),data=df_lines,color="orange",size=.1)+
  #geom_vline(aes(xintercept=med),data=summary_inf_subset,color="red",size=.1)+
  #geom_vline(aes(xintercept=mu),data=mus,color="blue",size=.1)+
