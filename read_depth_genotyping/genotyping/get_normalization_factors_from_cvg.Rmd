...
```
# Code to get normalization factors for read depth genotyping
...
```
```{r}
t_wnd_cvg %>% dplyr::select(sample) %>% unique() %>% tail()
```
```{r}
...
# d=24816.00
# 3*u+d
summary_inf %>% filter(grepl("SGDP",sample))
```
...
select_samples = (summary_inf %>% filter(grepl("SGDP",sample)))$sample[1:10]
#select_samples = (summary_inf %>% filter(grepl("SGDP",sample)))$sample
