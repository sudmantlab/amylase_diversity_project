...
to_gt_AMY2A = t_gt %>% filter(name=="AMY2A") %>% dplyr::select(sample,gt) %>% filter(grepl("1KG",sample))
to_gt_AMY2B = t_gt %>% filter(name=="AMY2B") %>% dplyr::select(sample,gt) %>% filter(grepl("1KG",sample))
to_gt_AMY1 = t_gt %>% filter(name=="amy1_sum") %>% dplyr::select(sample,gt) %>% filter(grepl("1KG",sample))
```
```{r}
###FUNCTIONS
...
AMY1_gts = genotype(to_gt_AMY1, 20, "AMY1")
AMY2B_gts = genotype(to_gt_AMY2B, 20, "AMY2B")
AMY2A_gts = genotype(to_gt_AMY2A, 20, "AMY2A")

all_gts = rbind(AMY1_gts$final_resps,
                AMY2B_gts$final_resps,
                AMY2A_gts$final_resps)


outdir="/Users/petersudmant/Documents/science/sudmantlab/projects/ancient_DNA/amylase/amylase_diversity_project/read_depth_genotyping/genotyping/output"
fn_gts = paste(outdir,"/genotypes.likelihoods.tsv",sep="")
write.table(all_gts, fn_gts, sep="\t",row.names = FALSE)
...
g=ggplot(best_cp)
g+geom_histogram(aes(x=gt,fill=factor(cp)),binwidth=.05)+
  theme_bw()+
  scale_fill_manual(values=sample(viridisLite::turbo(18)))+
