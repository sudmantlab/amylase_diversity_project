...
labels = t %>% ungroup() %>% 
  ...
  mutate(txt = case_when(
                      contig == "chr1_chm13_103304997_103901127_0" ~ "chm13",
                      contig == "chr1_hg38_103456064_103863972_0" ~ "hg19/38",
                      contig == "HG00438#2#JAHBCA010000012.1_51513023_51826723_0" ~ "Anc",
                      TRUE ~ "")) %>%
...
p = ggtree(struc_phy) %<+% df_tip_data +
  geom_tippoint(aes(size=n),position=position_nudge(x=.02,y=0))+
  scale_size_continuous(range=c(.25,2))+
  hexpand(.1) + 
  geom_cladelab(data=labels,mapping=aes(node=node, label=txt),fontsize=1,offset=0.1,hjust=1)+
  theme(legend.position = "None")
