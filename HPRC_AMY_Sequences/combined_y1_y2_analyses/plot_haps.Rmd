...
fn_out = paste(outdir,"/haplotype_all_structures_plotinfo.tsv",sep="")
write.table(t,fn_out,sep="\t",row.names = FALSE,quote=FALSE)
...
gene_counts_per_hap %>% ungroup() %>%dplyr::select(name,n) %>% arrange(name,n) %>% unique()
...
representative_haps %>% arrange(n)
representative_haps %>% summarize(sum(n))
...
total = sum(representative_haps$n)

#hg38_and_similar = 13+18+14
hg38_and_similar = 13+18+1+14+1+1
hg38_and_similar/total

H5 = 3+1+3+2+1+1+1+1+1
H5/total
...
t_graph_clusters = read.table("/Users/petersudmant/Documents/science/sudmantlab/projects/ancient_DNA/amylase/amylase_diversity_project/HPRC_AMY_Sequences/combined_y1_y2_analyses/graph_genotyping_clusters/output/20230718_dendrogram.jaccard.bestcut_clusters_processed.tsv",header=TRUE,sep="\t",comment="") %>%
  mutate(contig=sample)
...
paf_plot_genes_clustered$cluster = factor(paf_plot_genes_clustered$cluster,levels = rev(c(1,3,4,0,10,7,6,8,2,5,9)))
...
g=g+geom_gene_arrow(aes(xmin=rel_start2,
    ...
    geom_gene_arrow(aes(xmin=tstart,
                    ...
                    position=position_nudge(x=0,y=.2),
                    size=.1,
                    data = paf_plot_genes_clustered %>% filter(contig %in% cluster_rep_indivs$contig))+
  geom_text(aes(x=tstart,
                y=contig,
                label=name,
                color=name),
...
pdf(fn_out,width=3.5,height=1.5)
print(g+facet_grid(cluster~.,scales="free_y",space = "free"))
dev.off()
...
t_ww_hap_freqs = read.table("/Users/petersudmant/Documents/science/sudmantlab/projects/ancient_DNA/amylase/amylase_diversity_project/graph_genotyping/assess_graph_genotypes/output/world_wide_cluster_freqs.tsv",header=T,sep="\t")
...
hap_names = data.frame(cluster=seq(0,10),
  ...
  inner_join(t_ww_hap_freqs, on="cluster") %>%
  mutate(label_hap = hap_names) %>%
  mutate(label_freq = signif(proportion,2))


order = rev(c(1,3,4,0,10,7,6,8,2,5,9))
order = rev(c(4,1,9,3,0,6,8,2,5,10,7))
t_clustered$cluster = factor(t_clustered$cluster,levels = order)
paf_plot_genes_clustered$cluster = factor(paf_plot_genes_clustered$cluster,levels = order)
hap_names$cluster = factor(hap_names$cluster,levels = order)
...
g=g+geom_gene_arrow(aes(xmin=rel_start2,
 ...
 geom_gene_arrow(aes(xmin=tstart,
  ...
  geom_text(aes(x=0,
            ...
            size=1.5,
            hjust=0,
            position=position_nudge(x=-50000,y=nudge),
            parse=TRUE)+
  geom_text(aes(x=0,
                y=cluster,
                label=label_freq),
            ...
            size=1,
            hjust=0,
            position=position_nudge(x=-50000,y=0),
            parse=TRUE)+
  theme_bw(base_size=4)+
  theme(legend.position="None")+
  scale_fill_manual(values=colors)+
...
fn_out = paste(outdir,"/graph_clusters/haplotype_structures_graph_clusters_representative_alt2.pdf",sep="")
