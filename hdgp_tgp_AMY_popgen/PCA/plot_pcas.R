...
#p2_nonfacet
plot_pca_POPULATIONS<-function(eigenvector, eigenvalue, metadata, mytitle){
  pca <- read_table(eigenvector, col_names = FALSE)
  eigenval <- scan(eigenvalue,)
  pca <- pca[,-1]
  names(pca)[1] <- "ID"
...
b0<-plot_pca_POPULATIONS("combined_bundle0.eigenvec", "combined_bundle0.eigenval", "CN_metadata_combined.tsv", "AMY left flank (bundle 0)")
b1a<-plot_pca_POPULATIONS("combined_bundle1a.eigenvec", "combined_bundle1a.eigenval", "CN_metadata_combined.tsv","AMY right flank (bundle 1a)")

ED2C<-ggarrange(b0, b1a, labels = c("A", "B"), ncol = 2, nrow = 1, common.legend = TRUE, legend = "bottom")
print(ED2C)

ggsave("FigED2_C_sup_pop_pca_b0b1a.pdf", ED2C, width=7, height=3, dpi=300) 
