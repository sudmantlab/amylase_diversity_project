...
subsistence_types = ['Agriculture', 'Hunting', 'Pastoralism']
unique_pairs_subsistence_types = list(combinations(subsistence_types, 2))
...
rule all:
    input: 
        expand("selection_stats/SUBSISTENCE/{chrom}_combined.{anything}", chrom=chromosomes, anything=extensions),
        expand("selection_stats/SUBSISTENCE/genomewide.{lassipstat}.lassip.hap.stats.gz", lassipstat=lassipstats),
        expand("selection_stats/SUBSISTENCE/genomewide_combined.{anything}", anything=extensions),
...
rule vcf_all_genotype_individuals: # subset vcf per all individuals with CN estimates not trios
    ...
    wildcard_constraints:
        superpopulation="|".join(subsistence_types),
...
rule vcf_all_genotype_anything:  # subset vcf per pop of interest with CN estimates not trios
    input:
        ...
        'groups/subsistence/{superpopulation}.txt'
    output:
...
rule lassip_Hstats: 
    input:
        ...
        popfile = "groups/subsistence/{superpopulation}.lassip.txt"
    output:
...
rule concat_withingroup_tables:
    input:
        expand("{{chrom}}_{superpopulation}.{{anything}}", superpopulation=subsistence_types),
    output:
        temp("selection_stats/SUBSISTENCE/{chrom}_combined.{anything}"),
    run:
...
rule concat_betweengroups_tables:
    input:
        expand("{{chrom}}_{pair[0]}_{pair[1]}.{{anything}}", pair=unique_pairs_subsistence_types),
    output:
        "selection_stats/SUBSISTENCE/{chrom}_combined.{anything}",
    run:
        pd.concat([pd.read_csv(path, sep="\t") for path in input], ignore_index=True).to_csv(output[0], sep="\t", compression='gzip', index=False)

rule concat_tables_genomewide:
    input:
        expand("selection_stats/SUBSISTENCE/{chrom}_combined.{{anything}}", chrom=chromosomes),
    output:
...
rule concat_lassisalti:
    input:
        expand("{superpopulation}.{{lassipstat}}.lassip.hap.stats.gz", superpopulation=subsistence_types),
    output:
        "selection_stats/SUBSISTENCE/genomewide.{lassipstat}.lassip.hap.stats.gz",
    run:
        pd.concat([pd.read_csv(path, sep="\t") for path in input], ignore_index=True).to_csv(output[0], sep="\t", compression='gzip', index=False)
